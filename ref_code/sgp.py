...
class PPO_SGP():
            ...
            for sample in data_generator:
                ...
                if self.custom_adam == True:
                    ...
                    if task_num == 0:
                        ...
                        self.optimizer.update_params_projected(self.actor_critic,self.iteration,self.feature_mat, self.feature_list)
    ...
    def get_representation_matrix(self, rollouts, task_num, collect_act=True):
        ...
        mat_list = [[], [], [], []]  ## layerwise activation for GPM
        advantages = rollouts.returns[:-1] - rollouts.value_preds[:-1]
        ...
        for sample in data_generator:
            obs_batch, recurrent_hidden_states_batch, _, \
                _, _, masks_batch, _, adv_targ = sample

            # -----------------------------
            # 1. 用 |adv| 做 top-k sample selection
            # -----------------------------
            # if getattr(self, "weighted_gpm", False):
            #     scores = adv_targ.abs().view(-1)   # [bsz]
            #     k_keep = max(1, int(self.gpm_topk_ratio * scores.numel()))
            #     top_idx = torch.topk(scores, k_keep).indices

            #     obs_batch = obs_batch[top_idx]
            #     recurrent_hidden_states_batch = recurrent_hidden_states_batch[top_idx]
            #     masks_batch = masks_batch[top_idx]
            scores = adv_targ.abs().view(-1)
            k_keep = max(1, int(0.25 * scores.numel()))
            top_idx = torch.topk(scores, k_keep).indices
            obs_batch = obs_batch[top_idx]
            recurrent_hidden_states_batch = recurrent_hidden_states_batch[top_idx]
            masks_batch = masks_batch[top_idx]
            ...
            # forward to collect activations
            ...
            for i in range(len(self.actor_critic.features.map)):
                ...
                if i < 3:
                    ksz = self.actor_critic.features.ksize[i]
                    s = compute_conv_output_size(
                        self.actor_critic.features.map[i],
                        self.actor_critic.features.ksize[i],
                        self.actor_critic.features.stride[i]
                    )
                    mat = np.zeros((
                        self.actor_critic.features.ksize[i] *
                        self.actor_critic.features.ksize[i] *
                        self.actor_critic.features.in_channel[i],
                        s * s * bsz
                    ))
